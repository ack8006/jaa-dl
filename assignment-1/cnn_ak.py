...
class ConvNet(torch.nn.Module):
    def __init__(self, output_dim, dropout1=0.25, dropout2=0.5):
        super(ConvNet, self).__init__()

        self.conv = torch.nn.Sequential()
        self.conv.add_module('conv_1', torch.nn.Conv2d(1, 32, kernel_size=(3, 3)))
        self.conv.add_module("relu_1", torch.nn.ReLU())
        self.conv.add_module('conv_2', torch.nn.Conv2d(32, 32, kernel_size=(3, 3)))
        self.conv.add_module("relu_2", torch.nn.ReLU())
        self.conv.add_module("maxpool_1", torch.nn.MaxPool2d(kernel_size=(2, 2)))
        self.conv.add_module("dropout_1", torch.nn.Dropout2d(p=dropout1))

        self.fc = torch.nn.Sequential()
        self.fc.add_module("fc1", torch.nn.Linear(4608, 128))
        self.fc.add_module("relu_3", torch.nn.ReLU())
        self.fc.add_module("dropout_2", torch.nn.Dropout(p=dropout2))
        self.fc.add_module("fc2", torch.nn.Linear(128, output_dim))
...
def main():
    ...
    n_classes = 10
    model = ConvNet(output_dim=n_classes, dropout1=0.25, dropout2=0.5)
    ...
    for i in range(epochs):
        ...
        model_infor = 'b{}e{}acc{}'.format(str(batch_size),
                                           str(i),
                                           str(validation_accuracy).replace('.', ''))

        if validation_accuracy > best_validation_accuracy:
            best_validation_accuracy = validation_accuracy
            with open("saved_models/best_cnn_ak_{}.model".format(model_infor), "w") as file_pointer:
