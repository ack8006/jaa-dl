...
class Net(nn.Module):
    ...
    def forward(self, x):
        x = F.leaky_relu(F.max_pool2d(self.conv1(x), 2), negative_slope=1/5.5)
        x = F.leaky_relu(F.max_pool2d(self.conv2_drop(self.conv2(x)), 2), negative_slope=1/5.5)
        x = x.view(-1, 320)
        x = F.leaky_relu(self.fc1(x), negative_slope=1/5.5)
        x = F.dropout(x, training=self.training)
        x = F.leaky_relu(self.fc2(x), negative_slope=1/5.5)
...
for epoch in range(1, epochs + 1):
    train(epoch)
    val_acc = test(epoch, valid_loader)
    model_infor = 'e{}acc{}'.format(str(i), str(validation_accuracy).replace('.',''))
    if val_acc > best_acc:
        best_acc = val_acc
        with open("saved_models_simple/best_simple_{}.model".format(model_infor), "w") as file_pointer:
